...
const ViewPOExcelSheetData = () => {
  const [records, setRecords] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [expandedRow, setExpandedRow] = useState(null); // expanded WO index
  const [search, setSearch] = useState("");

  // ✅ Fetch from Firestore collection: "excelupload"
  useEffect(() => {
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <p className="text-xs text-slate-400 mt-0.5">
            All work orders saved via Excel upload · {records.length} total
          </p>
        </div>
...
export default ViewPOExcelSheetData;
