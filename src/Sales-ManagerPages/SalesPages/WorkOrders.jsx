    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">{WORK_ORDERS.length} total work orders</p>
        </div>
          ...
          <div className="flex-1 grid grid-cols-2 gap-3 max-w-md">
            <Select
              ...
              options={[
                ...
                ...Object.keys(WO_STATUSES).map(k => ({ value: k, label: WO_STATUSES[k].label }))
              ]}
            />
            <Select
              label="Priority"
            ...
            <tbody className="divide-y divide-slate-50">
              {filtered.map(wo => (
                <tr key={wo.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-4 py-3.5 text-xs text-slate-600 max-w-[150px] truncate">{wo.customer}</td>
                  <td className="px-4 py-3.5 text-center"><StatusBadge status={wo.priority} /></td>
                  <td className="px-4 py-3.5 text-center"><StatusBadge status={wo.mode} /></td>
