...
import { Card, CardHeader, StatusBadge, BtnSecondary } from "../SalesComponent/ui/index";
...
export default function WorkOrderDetails() {
  ...
  if (!wo) return <div className="text-center py-20 text-slate-400">Work Order not found</div>;

  return (
    <div className="space-y-6 max-w-5xl">
      <div className="flex items-center justify-between">
        <button onClick={() => navigate("/sales/work-orders")} className="flex items-center gap-2 text-sm font-semibold text-slate-600 hover:text-slate-800">
          <FiArrowLeft size={16} /> Back to Work Orders
        </button>
        <div className="flex gap-2">
          {wo.status === "draft" && (
            <BtnSecondary onClick={() => navigate(`/sales/work-orders/create?edit=${wo.id}`)}>
              <FiEdit size={14} /> Edit
            </BtnSecondary>
          )}
          {wo.attachment && (
      ...
      <Card className="p-6">
        <div className="flex items-start justify-between">
          <div>
            <h2 className="text-2xl font-black text-slate-800">{wo.id}</h2>
            <p className="text-sm text-slate-500 mt-1">Created by {wo.createdBy} on {wo.date}</p>
          </div>
      ...
      {/* Items */}
      <Card>
        <CardHeader title="Product Items" subtitle={`${wo.items.length} item(s)`} />
      ...
      {(wo.status === "approved" || wo.status === "rejected") && (
        <Card>
          <CardHeader title={wo.status === "approved" ? "Approval Information" : "Rejection Information"} />
